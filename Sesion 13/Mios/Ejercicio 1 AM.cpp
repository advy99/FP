...
public:
	...
	void Suma( Dinero cantidad_sumar){
		Dinero suma_dineros;
		
		euros = euros + cantidad_sumar.SaldoEuros();
		centimos = centimos + cantidad_sumar.SaldoCentimos();
		CuadrarSaldo();
	}	
};
...
int main(){
	...
	cuenta.Suma(monedero);

	cout << "Tinene un total de: ";
	cout << cuenta.PrintDinero();
}
